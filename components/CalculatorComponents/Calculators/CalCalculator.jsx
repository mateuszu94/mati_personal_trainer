...
const CalCalculator = ({ SelectedType, useRouter }) => {
  ...
  const getCal = () => {
    if (gender === "Mężczyzna") {
      const ppmValue = [
        (66.5 + 13.75 * weight + 5.003 * height - 6.775 * age).toFixed(0),
        10 * weight + 6.25 * height + 5 * age + 5,
      ];
      Ppm = ppmValue;
    } else {
      const ppmValue = [
        (655.1 + 9.563 * weight + 1.85 * height - 4.676 * age).toFixed(0),
  ...
  const getCpm = () => {
    ...
    if (matchedItem) {
      Cpm = [matchedItem.multiplier * Ppm[0], matchedItem.multiplier * Ppm[1]];
    }
  };
  ...
  return (
    <div className="calculator   bg-white">
      ...
      <form onSubmit={handleSubmit}>
        ...
        />
        <SelectObjective setObjective={setObjective} objective={objective} />

        <div className=" flexCenter ">
          <Button
            disabled={gender == "" || height == 0 || age == 0 || weight == 0}
            type="submit"
