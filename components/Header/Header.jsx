...
export const Header = () => {
  ...
  return (
    ...
    animate={{  opacity: isScrolling ? 0.90 : 1  , height: isScrolling ? `3rem` : `5rem`}} 
    whileHover={{ opacity:1}}
    
    transition={{ duration: 0.3 }}
     className={` relative z-50 bg-black flex justify-center    h-20  ${isScrolling ? " sticky  top-0" : ""}`}>
      <motion.div 
      initial={{ width: `80%`, height: `5rem`, translateY : 16 }} 
      animate={{ width: isScrolling ? `100%` : `80%` ,height: isScrolling ? `3rem` : `5rem`  , translateY: isScrolling ? 0 : 16}} 
      transition={{ duration: 0.3 }}
      className={`bg-prime  ${isScrolling ?  "rounded-none" : "rounded-xl" }   flex h-20 border-accentColor border-b-2  `}>
